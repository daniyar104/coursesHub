...
export default function CourseCardProfile({ course }: CourseCardProps) {
    ...
    return (
        <div className="flex flex-col justify-between bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition duration-300 border border-gray-100">
            ...
            {/* Категория */}
            <p className="text-sm text-gray-500 mb-3">{course.categories?.name}</p>
            ...
            <div className="flex flex-col h-15 gap-2 mt-auto mb-2.5">
                ...
                <p className="text-sm font-medium text-gray-600">{progress}% выполнено</p>
            </div>

            {/* Кнопка */}
            <button
                className="mt-4 w-full py-2 px-4 text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 transition duration-150 cursor-pointer shadow-md"
                onClick={() => {
