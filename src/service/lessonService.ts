...
export async function getLessonMaterial(lessonId: string): Promise<LessonMaterial> {
    ...
    try {
        ...
        console.error("Error fetching lesson material:", error.response?.status, error.response?.data);
        throw error;
    }
}
...
export async function markLessonAccess(courseId: string, lessonId: string): Promise<void> {
    ...
    try {
        await api.post(`/courses/${courseId}/lessons/${lessonId}/access`, {}, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
...
export async function markLessonComplete(courseId: string, lessonId: string): Promise<{ message: string; progress: number }> {
    const token = getToken();
    ...
    try {
        ...
        console.error("Error marking lesson complete:", error.response?.status, error.response?.data);
        throw error;
    }
}
